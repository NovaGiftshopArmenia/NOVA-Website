...
const TRANSLATIONS = {
  am: {
    ...
    hero_title: "Քո բույրը քեզ է սպասում",
  ...
  ru: {
    ...
    maison_de_parfum: "Maison de Parfum",
    hero_title: "Твой аромат ждет тебя",
...
window.openDetailedProductModal = function(productId) {
  ...
  // Set dropdowns
  document.getElementById('modal-product-family').value = product.scent_family || 'woody';
  ...
  // Set notes
  document.getElementById('modal-notes-top').value = product.notes && product.notes.top ? product.notes.top.join(', ') : '';
  ...
  if (product.image) {
    window._modalUploadedImages.push(product.image);
  }
  if (product.images && Array.isArray(product.images)) {
    product.images.forEach(img => {
      if (!window._modalUploadedImages.includes(img)) {
        window._modalUploadedImages.push(img);
      }
    });
  }
...
window.saveDetailedProduct = function(event) {
  ...
  const newBrand = document.getElementById('modal-product-brand').value.trim();
  const newTagline = document.getElementById('modal-product-tagline').value.trim();
  const newDescription = document.getElementById('modal-product-description').value.trim();
  const newIngredients = document.getElementById('modal-product-ingredients').value.trim();
  const newImage = document.getElementById('modal-product-image').value.trim();
  const newScentFamily = document.getElementById('modal-product-family').value;
